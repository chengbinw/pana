...
#   ./run_analysis_auto.sh TWOFISH BLOWFISH BYE    Run multiple simulations (3+)
#   ./run_analysis_auto.sh TWOFISH BLOWFISH --compare  Run multiple and compare
#   ./run_analysis_auto.sh TWOFISH BLOWFISH BYE --compare  Run multiple and compare (3+)
    ...
    if [ ${#SUCCESSFUL_SIMS[@]} -lt 2 ]; then
        ...
        if [ "$ALL_YEARS" = true ]; then
            ...
            if [ $? -ne 0 ]; then
                ...
                # Generate reports for each year directory created by compare_simulations.py
                ...
                # Also generate overall comparison report (all years combined)
                echo ""
                echo "Generating overall comparison report (all years combined)..."
                run_comparison_for_year "all" ""
                if [ $? -ne 0 ]; then
                    echo "  ERROR: Overall comparison report generation failed"
                else
                    echo "  ✓ Overall comparison report generated"
                fi
            fi
        else
...
if [ "$COMPARE_MODE" = true ] && [ ${#SUCCESSFUL_SIMS[@]} -ge 2 ]; then
    ...
    if [ "$ALL_YEARS" = true ]; then
        ...
        year_count=${#year_dirs[@]}
        # Print year directories (if any)
        for ((i=0; i<year_count; i++)); do
            year_dir="${year_dirs[$i]}"
            year=$(basename "$year_dir" | sed 's/comparison_//')
            # If there are year directories AND we will also show overall comparison directory,
            # all year directories use ├── prefix
            echo "    ├── comparison_$year/"
            echo "    │   ├── comparison_plots/"
            echo "    │   ├── comparison_data/"
            echo "    │   └── comparison_report.{md,html}"
        done
        # Always show overall comparison directory (all years combined) as the last entry
        if [ $year_count -gt 0 ]; then
            echo "    └── comparison/"
        else
            # No year directories, show only overall comparison
            echo "    └── comparison/"
        fi
        echo "        ├── comparison_plots/"
        echo "        ├── comparison_data/"
